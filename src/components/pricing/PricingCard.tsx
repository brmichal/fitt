...
export const PricingCard = ({ 
  ...
  const handleGetStarted = () => {
    const planType = title.toLowerCase().replace(' ', '-');
    navigate(`/programs/${planType}-registration`);
  };
  ...
  return (
    <div className={`relative p-6 rounded-xl ${gradient} text-white transition-transform hover:scale-105 duration-300 h-full flex flex-col ${isPopular ? 'ring-2 ring-orange-400' : ''}`}>
      ...
      <Button 
        className="w-full mt-6 bg-secondary hover:bg-secondary/90 text-primary font-semibold flex items-center justify-center gap-2"
        onClick={handleGetStarted}
      >
        Get Started
        {title === "Transformation Plan" && <Flame className="w-4 h-4 text-orange-500 fill-orange-500" />}
      </Button>
    </div>
  );
};